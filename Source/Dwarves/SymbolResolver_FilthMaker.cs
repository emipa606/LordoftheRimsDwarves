            ...
            if (rp.streetHorizontal != null && rp.streetHorizontal.Value == true)
	        {
		        ...
		        foreach (var c in ellipse)
		        {
			        if (rp.chanceToSkipFloor == null || !Rand.Chance(rp.chanceToSkipFloor.Value))
			        {
				        ...
				        if (rp.filthDef != null)
				        {
					        FilthMaker.TryMakeFilth(c, map, rp.filthDef, (rp.filthDensity == null) ? 1 : Mathf.RoundToInt(rp.filthDensity.Value.RandomInRange));
				        }
			        }
		        }   
	        }
	        else
	        {
		        ...
		        while (!iterator.Done())
		        {
			        if (rp.chanceToSkipFloor == null || !Rand.Chance(rp.chanceToSkipFloor.Value))
			        {
				        ...
				        if (rp.filthDef != null)
				        {
					        FilthMaker.TryMakeFilth(iterator.Current, map, rp.filthDef, (rp.filthDensity == null) ? 1 : Mathf.RoundToInt(rp.filthDensity.Value.RandomInRange));
				        }
			        }
			        iterator.MoveNext();
		        }   
